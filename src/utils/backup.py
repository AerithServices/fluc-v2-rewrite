...
async def create_backup(guild: discord.Guild) -> BackupData:
...
async def load_backup(guild: discord.Guild, backup: BackupData) -> bool:
    ...
    for scheduled_event in backup['scheduled_events']:
        ...
        _scheduled_event: discord.ScheduledEvent = await guild.create_scheduled_event( # pyright: ignore[reportCallIssue]
            ...
            end_time=end_time and datetime.fromtimestamp(end_time) or MISSING,
...
async def clear_server(guild: discord.Guild):
    await guild.edit(
        name=f'{guild.owner}\'s server',
        community=False,
        description=None,
        icon=None,
        banner=None,
    ...
    await guild.edit(
        ...
        dms_disabled_until=utils.now(seconds=3),
        invites_disabled_until=utils.now(seconds=3)
    )
    for role in guild.roles:
